...
static int QuickBody(int *array, int left, int right) {
    int i, j, pivot;
...
void Qselect(int *array, int left, int right, int number) {
    int i;
    if (left + 10 < right) {
        i = QuickBody(array, left, right);
        if(number < i)
...
void QuickSelect(int *array, int len, int number) {
    Qselect(array, 0, len - 1, number - 1);
}
